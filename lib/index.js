...
exports.default = function (_ref) {
    ...
    function storeMessage(descriptor, path, state) {
        ...
        var defaultMessage = descriptor.defaultMessage.trim();
    ...
    function referencesImport(path, mod, importedNames) {
        var scoped = importedNames.filter(function (name) {
            return name.includes('.');
        }).map(function (name) {
            return name.split('.');
        });
        var standard = importedNames.filter(function (name) {
            return !name.includes('.');
        });
        if (!(path.isIdentifier() || path.isJSXIdentifier())) {
            if (path.isJSXMemberExpression()) {
                ...
                if (obj.isJSXIdentifier() && obj.referencesImport(mod)) {
                    if (property.isJSXIdentifier()) {
                        return standard.some(function (name) {
                            return property.node.name === name;
                        }) || scoped.some(function (_ref7) {
                            var _ref8 = (0, _slicedToArray3.default)(_ref7, 2),
                                scope = _ref8[0],
                                name = _ref8[1];

                            return property.node.name === name && obj.node.name === scope;
                        });
                    }
                }
                if (obj.isJSXMemberExpression()) {
                    if (obj.get('object').referencesImport(mod)) {
                        var scopeProperty = obj.get('property');
                        if (scopeProperty.isJSXIdentifier()) {
                            return scoped.some(function (_ref9) {
                                var _ref10 = (0, _slicedToArray3.default)(_ref9, 2),
                                    scope = _ref10[0],
                                    name = _ref10[1];

                                return property.node.name === name && scopeProperty.node.name === scope;
                            });
                        }
                    }
                }
    ...
    function isAdditionalComponent(it) {
        return (typeof it === 'undefined' ? 'undefined' : (0, _typeof3.default)(it)) === 'object' && typeof it.moduleSourceName === 'string' && Array.isArray(it.componentNames) && it.componentNames.length > 0;
    }
    ...
    function referencesAdditionalImport(path, additionalComponents) {
        if (Array.isArray(additionalComponents)) {
            if (!additionalComponents.every(isAdditionalComponent)) {
                throw path.buildCodeFrameError('[React Intl] Additional Components need to be in shape of ' + '`[{"moduleSourceName": "moduleName", "componentNames": ["ComponentName"]}]`' + ('but got:\n ' + (0, _stringify2.default)(additionalComponents, null, 2)));
            }
            return additionalComponents.some(function (_ref11) {
                var moduleSourceName = _ref11.moduleSourceName,
                    componentNames = _ref11.componentNames;
    ...
    return {
        ...
        visitor: {
            JSXOpeningElement: function JSXOpeningElement(path, state) {
                ...
                var additionalComponents = getAdditionalComponents(opts);
