...
exports.default = function (_ref) {
    ...
    function referencesScopedImport(path, mod, importedNames) {
        var scopeName = path.node.object && path.node.object.name;
        var componentName = path.node.property && path.node.property.name;
        ...
        return importedNames.some(function (name) {
            return name === scopeName + '.' + componentName;
        });
    }
    ...
    function referencesAdditionalImport(path, additionalComponents) {
        if (Array.isArray(additionalComponents)) {
            ...
            return additionalComponents.some(function (_ref7) {
                var moduleSourceName = _ref7.moduleSourceName,
                    componentNames = _ref7.componentNames;

                var scoped = componentNames.filter(function (name) {
                    return name.includes('.');
                });
                var other = componentNames.filter(function (name) {
                    return !name.includes('.');
                });
