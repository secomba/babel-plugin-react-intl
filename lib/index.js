...
function _default({
      ...
      if (path.isJSXMemberExpression()) {
        ...
        if (obj.isJSXIdentifier() && obj.referencesImport(mod)) {
          if (property.isJSXIdentifier()) {
            return standard.some(name => property.node.name === name) || scoped.some(([scope, name]) => property.node.name === name && obj.node.name === scope);
          }
        }
        ...
        if (obj.isJSXMemberExpression()) {
          if (obj.get('object').referencesImport(mod)) {
            ...
            if (scopeProperty.isJSXIdentifier()) {
              return scoped.some(([scope, name]) => property.node.name === name && scopeProperty.node.name === scope);
            }
          }
        }
      }
  ...
  function isAdditionalComponent(it) {
    return typeof it === 'object' && typeof it.moduleSourceName === 'string' && Array.isArray(it.componentNames) && it.componentNames.length > 0;
  }
  ...
  return {
    ...
    visitor: {
      JSXOpeningElement(path, state) {
        ...
        if (referencesImport(name, moduleSourceName, DEFAULT_COMPONENT_NAMES) || additionalComponentNames.includes(name.node.name) || referencesAdditionalImport(name, additionalComponents)) {
