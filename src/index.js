      ...
      if (path.isJSXMemberExpression()) {
        ...
        if (obj.isJSXIdentifier() && obj.referencesImport(mod)) {
          if (property.isJSXIdentifier()) {
            return (
              standard.some(name => property.node.name === name) ||
              scoped.some(
                ([scope, name]) =>
                  property.node.name === name && obj.node.name === scope
              )
            );
          }
        }
    ...
    visitor: {
      JSXOpeningElement(path, state) {
        ...
        if (
          ...
          referencesAdditionalImport(name, additionalComponents)
        ) {
